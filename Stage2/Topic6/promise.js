...
function handleError(error){
    ...
    const listDiv = document.getElementById("list");
    listDiv.innerHTML = "An error occurred while fetching data.";
}
...
const myFetchPromise = myFetch();

console.log("myFetchPromise",myFetchPromise);


 myFetchPromise.then(()=>{
    console.log("Then: Promise resolved successfully");
    renderData();
 }).catch(()=>{
    console.log("Catch: Error occurred");
    handleError();
 }).finally(()=>{
    console.log("Finally: Promise settled (either resolved or rejected)");
 });
